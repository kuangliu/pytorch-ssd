...
class MultiBox(nn.Module):
    ...
    # output channels of layer [conv4_3, conv7, conv8_2, conv9_2, conv10_2, conv11_2]
    in_channels = [512,1024,512,256,256,256]

    def __init__(self):
        super(MultiBox, self).__init__()
        self.loc_layer_names = ['conv4_3_loc', 'conv7_loc', 'conv8_2_loc', 'conv9_2_loc', 'conv10_2_loc', 'conv11_2_loc']
        self.conf_layer_names = ['conv4_3_conf', 'conv7_conf', 'conv8_2_conf', 'conv9_2_conf', 'conv10_2_conf', 'conv11_2_conf']

        # Add prediction heads.
        self.conv4_3_loc = nn.Conv2d(512, 4*4, kernel_size=3, padding=1)
        self.conv4_3_conf = nn.Conv2d(512, 4*self.num_classes, kernel_size=3, padding=1)

        self.conv7_loc = nn.Conv2d(1024, 6*4, kernel_size=3, padding=1)
        self.conv7_conf = nn.Conv2d(1024, 6*self.num_classes, kernel_size=3, padding=1)

        self.conv8_2_loc = nn.Conv2d(512, 6*4, kernel_size=3, padding=1)
        self.conv8_2_conf = nn.Conv2d(512, 6*self.num_classes, kernel_size=3, padding=1)

        self.conv9_2_loc = nn.Conv2d(256, 6*4, kernel_size=3, padding=1)
        self.conv9_2_conf = nn.Conv2d(256, 6*self.num_classes, kernel_size=3, padding=1)

        self.conv10_2_loc = nn.Conv2d(256, 4*4, kernel_size=3, padding=1)
        self.conv10_2_conf = nn.Conv2d(256, 4*self.num_classes, kernel_size=3, padding=1)

        self.conv11_2_loc = nn.Conv2d(256, 4*4, kernel_size=3, padding=1)
        self.conv11_2_conf = nn.Conv2d(256, 4*self.num_classes, kernel_size=3, padding=1)
    ...
    def forward(self, xs):
        ...
        for i,x in enumerate(xs):
            loc_layer = getattr(self, self.loc_layer_names[i])
            ...
            conf_layer = getattr(self, self.conf_layer_names[i])
    ...
    def hard_negative_mining_bk(self, conf_loss, pos):
